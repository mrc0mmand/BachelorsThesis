...
\section{Jenkins}
    ...
    In our current infrastructure we use Beaker~(Chapter~\ref{ref:beaker}) for machine
    provision, where each tasks has its own machine. This sounded like exactly
    ...
    using OpenStack~(Chapter~\ref{ref:openstack}) for provision of virtual
    machines. OpenStack could run on the same machine as Jenkins, so we wouldn't
    ...
    Finally, we managed to isolate build tasks using Docker~\ref{ref:docker} containers.
...
\subsection{OpenStack} \label{ref:openstack}
    OpenStack is a cloud operating system for managing pools of compute, storage,
